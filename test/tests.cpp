...
TEST(task, test2_on_transitions_to_waiting) {
    ...
    EXPECT_NE(output.find("The machine is on"),
        std::string::npos);
...
TEST(task, test3_menu_prints_items) {
    ...
    automata.getMenu();
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_NE(output.find("0: Cappuccino - 120"),
        std::string::npos);
    EXPECT_NE(output.find("3: Latte - 140"),
        std::string::npos);
}
...
TEST(task, test4_invalid_choice_index) {
    ...
    automata.choice(-1);
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_NE(output.find("Incorrect drink index"),
        std::string::npos);
}
...
TEST(task, test5_coin_prints_and_updates) {
    ...
    auto pos = output.find(
        "You deposited 75, Current balance: 75");
...
TEST(task, test6_choice_valid_index) {
    ...
    automata.choice(1);
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_NE(output.find("You have chosen: Tea"),
        std::string::npos);
...
TEST(task, test7_check_insufficient_funds) {
    ...
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_FALSE(result);
    EXPECT_NE(output.find("Not enough money"),
        std::string::npos);
}
...
TEST(task, test8_check_sufficient_funds) {
    ...
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_TRUE(result);
    EXPECT_NE(output.find("preparations begin"),
        std::string::npos);
}
...
TEST(task, test9_cook_and_finish_prints) {
    ...
    automata.finish();
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_NE(output.find("Preparing the drink: Latte"),
        std::string::npos);
    EXPECT_NE(output.find("The drink is ready"),
        std::string::npos);
    EXPECT_NE(output.find("Your change is: 60"),
        std::string::npos);
}
...
TEST(task, test10_cancel_refund) {
    ...
    automata.cancel();
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_NE(output.find("Order cancelled, refund: 85"),
        std::string::npos);
    EXPECT_EQ(automata.getState(), STATES::WAITING);
}
